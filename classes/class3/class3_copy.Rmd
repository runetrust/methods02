...
```{r Live coding}
for (number in 1:10) {
  print(paste(number, "guy")) #Paste allows you to do string interpolation
  squaredNumber <- number^2
  print(squaredNumber)
}

list_names <- c("pernille", "nicolas", "anna")
for (name in list_names) {
  print(name)
}
```
```{r more live, while loop}
counter <- 1
while (counter <= 17) {
  print(paste("Counter is currently", counter))
  counter <- counter+1 #This is necessary for counter actually counting up, otherwise infinite loop
}
...
some_numbers <- c(-7,5,1,0,-67929)
number_looper(some_numbers)

```

```{r}
calculate_area <- function(width,
                           length_) {
  area <- width * length_
  return(area)
}
...
Hint A2* under Hints
```{r}
vector_squares <- c() #Creating an empty list to be input into the for loop
for (i in 1:20) {
  vector_squares[i] <- i^2 #This line indexes (places a number in a place in the vector) based on it's place (i), and assigns that value as i squared
}
print(sum(vector_squares))
...
while (sum_tracker < 1000){ #Will only run while sum_tracker object is smaller than 1000 
  sum_tracker <- sum_tracker+n #This adds n onto the sum_tracker object for each iteration
  n <- n+1 #Here 1 is added onto n for each iteration
  print(n-1)
}
```
...
```{r dice roll simulation}

die1 <- sample(1:6, 10000, replace = TRUE) #This samples through the possible lengths of 1 through 6 (the faces of the die) 10000 times, with replacement so that all values can be chosen again
die2 <- sample(1:6, 10000, replace = TRUE)

dice_sum <- die1+die2 #This just sums the two together

hist(dice_sum)
print(paste("Mean of dice rolls is ", mean(dice_sum)))
print(paste("Standard deviation of rolls is ", sd(dice_sum)))
