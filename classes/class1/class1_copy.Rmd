...
knitr::opts_chunk$set(root.dir = "C:/Uni/2. Semester/Methods 02/methods02/classes")
...
```{r}
setwd("C:/Uni/2. Semester/Methods 02/methods02/classes")
# Load data
...
```{r}
#Calculating mean for sim_data at the column "value"
mean_iq <- mean(sim_data$value)
mean_iq

#The same for standard deviation
sd_iq <- sd(sim_data$value)
sd_iq
```
...
```
```{r}
sample <- as_tibble(rnorm(20, 25, 10000000))
plot(sample)

ggplot(sample, aes(x = value)) +
  geom_histogram()
